...
function load_page($tree) {
	...
	if (isset($branch['type']) && $branch['type'] == 'file') {
		...
		include('setup.php');
			...
			$html .= '<ol class="breadcrumb"><li><a class="active" href="'.$Server_Link.'">'.$System_Name.'</a></li><li class="active">'. $branch['title'] . '</li></ol><hr/>';
...
function build_navo($tree, $url_params = false) {
	// Remove Index
	unset($tree['index']);
...
function build_nav($tree, $url_params = false) {
	...
	foreach($tree as $key => $val) {
		...
		if ($val['type'] == 'folder') {
			...
			$html .= '<a href="#" class="dropdown-toggle" data-toggle="dropdown">'.$val['name'].'</a>';
			
			$html .= ''.build_navo($val['tree'], $url_params).'';
		
		
		} else {
			$html .= '<a href="'.$val['url'].'">'.$val['name'].'</a>'; //v2
		}
